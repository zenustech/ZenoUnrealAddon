...
void UZenoLandscapeTool::Init()
{
	...
	Slate_DetailPanel->SetIsPropertyVisibleDelegate(
		FIsPropertyVisible::CreateUObject(this, &UZenoLandscapeTool::GetIsPropertyVisible));
	check(IsValid(UISetting));
	Slate_DetailPanel->SetObject(UISetting);

	AddMode(FName(NAME_ImportHeightfield), FZenoLandscapeCommand::Get().ImportLiveLinkHeightmap.ToSharedRef(),
	        FZenoEditorToolkitBuildToolPalette::CreateUObject(this, &UZenoLandscapeTool::Slate_MarkImportHeightfield));
	AddMode(FName(NAME_ExportWeightmap), FZenoLandscapeCommand::Get().ExportLiveLinkWeightmap.ToSharedRef(),
	        FZenoEditorToolkitBuildToolPalette::CreateUObject(this, &UZenoLandscapeTool::Slate_MarkExportWeightmap));
	AddMode(FName(NAME_VisualLandscapeLayer), FZenoLandscapeCommand::Get().VisualLayer.ToSharedRef(),
	        FZenoEditorToolkitBuildToolPalette::CreateUObject(this, &UZenoLandscapeTool::Slate_VisualLayer));
...
void UZenoLandscapeTool::ImportHeightMapFromSubject()
{
	...
	if (const TOptional<FLiveLinkSubjectFrameData> FrameData = FZenoCommonDataSource::GetFrameData(
		UISetting->SelectedSubjectKey); FrameData.IsSet())
	{
		const FLiveLinkHeightFieldStaticData* Data = FrameData->StaticData.Cast<FLiveLinkHeightFieldStaticData>();

		const int32 SizeX = UISetting->ImportLandscape_ComponentCount.X * UISetting->ImportLandscape_QuadsPerSection +
			1;
		const int32 SizeY = UISetting->ImportLandscape_ComponentCount.Y * UISetting->ImportLandscape_QuadsPerSection +
			1;
		...
		ALandscape* Landscape = GWorld->SpawnActor<ALandscape>(UISetting->NewLandscape_Location,
		                                                       UISetting->NewLandscape_Rotation);
		...
		Landscape->StaticLightingLOD = FMath::DivideAndRoundUp(FMath::CeilLogTwo((SizeX * SizeY) / (2048 * 2048) + 1),
		                                                       static_cast<uint32>(2));

		Landscape->Import(FGuid::NewGuid(), 0, 0, SizeX - 1, SizeY - 1, UISetting->ImportLandscape_SectionsPerComponent,
		                  UISetting->ImportLandscape_QuadsPerSection, HeightDataPerLayers,
		                  *FString::Printf(
			                  TEXT("zeno://%s.subject"), *UISetting->SelectedSubjectKey.SubjectName.ToString()),
...
void UZenoLandscapeTool::SaveHeightMapToAsset()
{
		...
		UISetting->SelectedSubjectKey); FrameData.IsSet())
	{
		...
		UZenoBridgeAsset* Asset = Cast<UZenoBridgeAsset>(
			AssetToolsModule.Get().CreateAssetWithDialog(UZenoBridgeAsset::StaticClass(), ZenoFactory));
		if (IsValid(Asset))
		{
		...
		UISetting->SelectedSubjectKey); FrameData.IsSet())
	{
		...
		for (ALandscapeProxy* LandscapeProxy : SelectedLandscapes)
		{
			if (ALandscape* Landscape = Cast<ALandscape>(LandscapeProxy); Landscape)
			{
				...
				{
					// Add zeno layer to modify the landscape
					...
					AZenoLandscapeSimpleBrush* Brush = FZenoLandscapeHelper::GetOrCreateLandscapeSimpleBrush(Landscape, ExistingWaterLayerIndex);
					if (IsValid(Brush))
					{
						Brush->SetBaseHeightmapData(HeightData);
					}
				}
			}
		}
